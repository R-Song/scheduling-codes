...
#include "chd_structs_n.h"
#include "chd_n.h"
#include "bitbool.h"
//#define DEBUG
#include "debug.h"

chd_n_config_data_t *chd_n_config_new(cmph_config_t *mph)
{
	cmph_io_adapter_t *key_source = mph->key_source;
	chd_n_config_data_t *chd;
	chd = (chd_n_config_data_t *)malloc(sizeof(chd_n_config_data_t));
        if (!chd) return NULL;
	memset(chd, 0, sizeof(chd_n_config_data_t));
...
void chd_n_config_destroy(cmph_config_t *mph)
{
	chd_n_config_data_t *data = (chd_n_config_data_t *) mph->data;
...
void chd_n_config_set_hashfuncs(cmph_config_t *mph, CMPH_HASH *hashfuncs)
{
	chd_n_config_data_t *data = (chd_n_config_data_t *) mph->data;
...
void chd_n_config_set_b(cmph_config_t *mph, cmph_uint32 keys_per_bucket)
{
	chd_n_config_data_t *data = (chd_n_config_data_t *) mph->data;
...
void chd_n_config_set_keys_per_bin(cmph_config_t *mph, cmph_uint32 keys_per_bin)
{
	chd_n_config_data_t *data = (chd_n_config_data_t *) mph->data;
...
cmph_t *chd_n_new(cmph_config_t *mph, double c, double a)
{
	DEBUGP("Creating new chd");
	cmph_t *mphf = NULL;
	chd_n_data_t *chdf = NULL;
	chd_n_config_data_t *chd = (chd_n_config_data_t *)mph->data;
	...
	mphf->algo = mph->algo;
	chdf = (chd_n_data_t *)malloc(sizeof(chd_n_data_t));
...
void chd_n_load(FILE *fd, cmph_t *mphf)
{
	register size_t nbytes;
	chd_n_data_t *chd = (chd_n_data_t *)malloc(sizeof(chd_n_data_t));
...
int chd_n_dump(cmph_t *mphf, FILE *fd)
{
	register size_t nbytes;
	chd_n_data_t *data = (chd_n_data_t *)mphf->data;
...
void chd_n_destroy(cmph_t *mphf)
{
	chd_n_data_t *data = (chd_n_data_t *)mphf->data;
...
static inline cmph_uint32 _chd_n_search(void * packed_chd_phf, void * packed_cr, const char *key, cmph_uint32 keylen)
{
...
cmph_uint32 chd_n_search(cmph_t *mphf, const char *key, cmph_uint32 keylen)
{
	register chd_n_data_t * chd = (chd_n_data_t *)mphf->data;
	return _chd_n_search(chd->packed_chd_phf, chd->packed_cr, key, keylen);
}

void chd_n_pack(cmph_t *mphf, void *packed_mphf)
{
	chd_n_data_t *data = (chd_n_data_t *)mphf->data;
...
cmph_uint32 chd_n_packed_size(cmph_t *mphf)
{
	register chd_n_data_t *data = (chd_n_data_t *)mphf->data;
	return (cmph_uint32)(sizeof(CMPH_ALGO) + 2*sizeof(cmph_uint32) + data->packed_cr_size + data->packed_chd_phf_size);

}

cmph_uint32 chd_n_search_packed(void *packed_mphf, const char *key, cmph_uint32 keylen)
{
	...
	return _chd_n_search(packed_chd_phf, ptr, key, keylen);
}
