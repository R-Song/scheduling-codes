				...
				cmph_uint32 nbuckets,cmph_uint32 item_idx)
{
    ...
    tmp_item->nack = tmp_map_item->nack;
	bucket->size++;
	return 1;
};
...
cmph_uint8 chd_ph_n_mapping(cmph_config_t *mph, chd_ph_bucket_t *buckets, chd_ph_item_t *items, cmph_uint32 *max_bucket_size)
{
    ...
    assert(chd_ph->m > chd_ph->nack);
    register cmph_uint32 nack = chd_ph->m - chd_ph->nack;
	*max_bucket_size = 0;
	while(1) {
		mapping_iterations--;
		if (chd_ph->hl) {
		...
		for(i = 0; i < chd_ph->m; i++) {
            ...
            // Identify if this item is part of nack set or not
            if(i >= nack) {
                map_item->nack = 1; 
            } else {
                map_item->nack = 0; 
                                                  ...
                                                  cmph_uint32 * disp_table, chd_ph_sorted_list_t * sorted_lists,cmph_uint32 max_bucket_size)
{
	...
	for(bucket_size = 1; bucket_size <= max_bucket_size; bucket_size++) {
		for(i = sorted_lists[bucket_size].buckets_list; 
                i < sorted_lists[bucket_size].size + sorted_lists[bucket_size].buckets_list; 
                i++)
		{
			j = bucket_size;
...
cmph_t *chd_ph_n_new(cmph_config_t *mph, double c, double a)
{
    ...
    // nkeys and keys_per_bucket are set during config creation
	chd_ph->m = mph->key_source->nkeys;
	DEBUGP("m = %u\n", chd_ph->m);
	chd_ph->nbuckets = (cmph_uint32)(chd_ph->m/chd_ph->keys_per_bucket) + 1;
	DEBUGP("nbuckets = %u\n", chd_ph->nbuckets);

    // Enforce a range on the load factor
	if(load_factor < 0.5 ) {
		load_factor = 0.5;
	} else if(load_factor >= 0.99) {
		load_factor = 0.99;
	}
    ...
    // Set the number of nacks and skew
    chd_ph->nack = mph->nack;
    chd_ph->a = mph->a;
